...
**Triage role mentions landed 2026-07-26**, closing the last parity gap (§3.3.7). An issue with nobody
assigned pings its workflow's `triageAlias`; once an owner or nominee exists the pings stop, because a
team notified about issues that already have a name against them mutes the channel. No escalation —
Slack's `escalateAlias` is driven by a timer Horde does not hand to a sink.

Three things came out of building it:

- **Roles had to become guild-scoped**, which is where this and multi-guild collide. A role id means
  nothing outside its own guild: mentioning one from elsewhere renders as raw `<@&id>` text and pings
  nobody, which reads as a formatting bug rather than a configuration one. The `roles` map is now
  `alias → { label, guild?, role }`, mirroring `channels`. **A config-shape change**, acceptable only
  because nothing could have been using the old one — no code path read it.
- **Aliases travel to the send path, not roles.** Which role an alias means depends on the guild being
  posted to, and one issue can reach channels in several; only `SendAsync` knows the destination.
- **`allowed_mentions` lists roles explicitly** rather than adding `roles` to `parse`, which would
  honour every role mention Discord can find in the text — including one that came out of a build log.

Verified live: `mention_roles` came back populated with `mention_everyone: false`, so the mention
resolved and the allowed-mentions shape was accepted.

**Multi-guild was already built.** The routing layer has had a `guilds` name→id map, per-channel
guilds, a default and a fallback guild since Phase 1, and every `DiscordDestination` carries its own
`GuildId`. `DiscordServerConfig.GuildId` turned out to be referenced in exactly one place — a startup
log line — so nothing in the posting path was ever guild-bound, as §4 intended. The only real gap was
roles, above.
...
- **Multi-guild** — *done, and largely already was.* The routing layer has carried named guilds since
  Phase 1; roles became guild-scoped in Phase 4. Multiple bot *tokens* remain out of scope.
